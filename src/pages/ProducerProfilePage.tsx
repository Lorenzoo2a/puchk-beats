  ...
  return (
    <div className="min-h-screen pt-16">
      ...
      <div className="h-48 relative" style={{ background: `linear-gradient(135deg, ${producer.genreColor}30, #141414)` }}>
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-puchk-base" />
      </div>

      <div className="max-w-5xl mx-auto px-6 -mt-16 relative z-10">
        {/* Profile header */}
        <div className="flex items-end gap-5 mb-8">
          <div className="w-24 h-24 rounded-full flex items-center justify-center text-3xl font-extrabold border-4 border-puchk-base" style={{ background: `linear-gradient(135deg, ${producer.genreColor}60, ${producer.genreColor}20)` }}>
            {producer.name.charAt(0)}
          </div>
          <div className="flex-1">
            <h1 className="text-3xl font-extrabold">{producer.name}</h1>
            <p className="text-sm text-white/40">@{producer.id}</p>
          </div>
          <button onClick={() => setFollowing(!following)} className={`px-5 py-2 rounded-xl text-sm font-bold spring-transition ${following ? "border border-puchk-success/30 text-puchk-success" : "border border-puchk-orange/30 text-puchk-orange hover:bg-puchk-orange/5"}`}>
            {following ? "Suivi ✓" : "Follow"}
          </button>
        </div>

        <p className="text-white/60 mb-4">{producer.bio}</p>
        ...
        <div className="flex gap-4 mb-10">
          {[{ v: producer.kits, l: "kits" }, { v: producer.sales.toLocaleString(), l: "ventes" }, { v: avgRating, l: "note moy." }, { v: producer.followers.toLocaleString(), l: "followers" }].map((s) => (
            <div key={s.l} className="px-3 py-2 rounded-xl bg-puchk-surface border border-[rgba(255,107,26,0.08)] text-center">
        ...
        <div className="mb-10">
          ...
          <div className="flex gap-3 mb-4">
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Laisse un commentaire..." rows={2} className="flex-1 px-4 py-3 bg-puchk-deep rounded-xl border border-[rgba(255,107,26,0.08)] text-sm text-white outline-none resize-none focus:border-puchk-orange/40 spring-transition" />
            <button className="px-4 py-2 bg-puchk-orange text-white text-sm font-bold rounded-xl hover:bg-puchk-orange-hover spring-transition self-end">Publier</button>
          </div>
          <div className="space-y-3">
            {[{ user: "xMelo", text: "Les kits sont incroyables, qualité dingue !", date: "il y a 3j" }, { user: "ProdNova", text: "Mon producteur préféré sur PUCHK.", date: "il y a 1 sem" }].map((c, i) => (
              <div key={i} className="glass rounded-xl p-4 flex gap-3">
                ...
                <div><div className="flex items-center gap-2 mb-1"><span className="text-sm font-bold">{c.user}</span><span className="text-[10px] text-white/30">{c.date}</span></div><p className="text-sm text-white/60">{c.text}</p></div>
              </div>
            ))}
          </div>
        </div>
